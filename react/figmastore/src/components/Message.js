        ...
        if (header[0] === "phone_number") {
          ...
          try {
            document.getElementById("alert-content").innerText =
              "Message Sending Sequence Started!";
            document.getElementById("successAlert").style.display = "block";
            setTimeout(() => {
              document.getElementById("successAlert").style.display = "none";
            }, 3000);
            const response = await axios.post(
              "http://localhost:3000/bulk-messages",
              data
            );
            console.log(response.data);
          ...
          document.getElementById("fail-alert-content").innerText =
            "The csv data is not in the mentioned format :";
    ...
    return(
        <div className='message-page-main'>
            ...
            <div className='individual-msg'>
                <form>
                <label>Message
                <input type='text' placeholder='enter text'></input></label>
      ...
      <div>
        ...
        <button id="upload" onClick={handleBulkMessageSend}>
          Send Bulk Messages
        </button>
        <div id="failAlert" className="alert fail">
          <span className="closebtn">&times;</span>
          <strong>Failed!</strong> <span id="fail-alert-content"></span>
        </div>
      </div>
    </div>
  );
